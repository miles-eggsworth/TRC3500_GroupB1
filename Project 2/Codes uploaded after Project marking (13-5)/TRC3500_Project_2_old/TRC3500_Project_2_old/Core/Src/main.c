...
// Custom functions by Jason
void CaptureSamples(void);
void CalculateMean(void);
void EstimateFrequency(float meanVoltage);
...
//    sprintf(msg, "Drop Detected! (Drop number: %d)\r\nMean Voltage = %.2f V\r\nEstimated Frequency = %.2f Hz\r\n",
//            dropCounter, meanVoltage, estimatedFrequency);
    sprintf(msg, "Coin D10 H30, Drop number: %d\r\n", dropCounter);
    dropCounter++;
    HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), HAL_MAX_DELAY);

    // Print the voltage samples
    // HAL_UART_Transmit(&huart2, (uint8_t*)"Voltage samples:\r\n[", strlen("Voltage samples:\r\n["), HAL_MAX_DELAY);
//    HAL_UART_Transmit(&huart2, (uint8_t*)"Voltage_samples:\r\n[\n", strlen("Voltage_samples\r\n"), HAL_MAX_DELAY);


    for (int i = 0; i < SAMPLE_COUNT; i++)
    {
        // Format each voltage sample as a float
        int len = sprintf(msg, "%.4f", voltageBuffer[i]);
